...
//------ main block ----------
void vn_EP_rapidity(const char* direct, int Npart_min, int Npart_max, double order)
{
  cout << endl << endl << "Calculating v_" << (int)order << "(rapidity)..." << endl;
  ...
  ofstream fout;
  fout.open("vn_EP_rapidity.dat", ofstream::app);
  ...
  cout << "Results have been written to 'vn_EP_rapidity.dat'" << endl;
